<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.jobJson" placeholder="脚本内容" style="width: 200px;" class="filter-item" />
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="60%" :before-close="handleClose">
      ...
      <div v-if="temp.glueType==='DATAX'" style="color: #606266;font-size: 14px;margin-bottom: 8px;">Datax Json Config&nbsp;<i class="el-icon-question" title="示例" style="cursor: pointer" @click="showDataxQ"></i></div>
    ...
    <el-dialog title="示例" :visible.sync="showQ" width="1000px">
      <!--<json-editor ref="showQEditor" v-model="jsonQ" />-->
      <shell-editor ref="showQEditor" v-model="jsonQ" />
    </el-dialog>
  </div>
</template>
  ...
  data() {
    ...
    return {
      ...
      jsonQ: '\n{\n' +
        ...
        '\t\t\t"LAST_UPDATE_TIME": "select update_time from t_user order by update_time desc limit 1"\n' +
        '\t\t},\n' +
        '\t\t"username": "root",\n' +
        '\t\t"password": "ody,123",\n' +
        '\t\t"jdbcUrl": "jdbc:mysql://10.10.0.48:3306/ouser?useUnicode=true&characterEncoding=utf-8&autoReconnect=true"\n' +
        '\t}],\n' +
        '\t\n' +
        '\t// datax 配置\n' +
        '\t"job": {\n' +
        '\t\t// 使用变量  ${CURRENT_TIME} ${LAST_UPDATE_TIME}\n' +
        '\t}\n' +
        '}\n'
    }
  },
  ...
  methods: {
    ...
    getJobIdList() {
      job.getJobIdList().then(response => {
        this.jobIdList = response.data
      })
    },
    getJobProject() {
      jobProjectApi.getProjectList().then(response => {
